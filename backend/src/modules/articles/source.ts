...
import {
  ...
  type ArticleLocale,
  type ArticleSection,
  type ArticleSourceDescriptor,
  type LoadedArticleDetail,
...
export function resolveArticleBucket(env: Env): R2Bucket {
...
export function normalizeArticleLocale(value: string | null | undefined): ArticleLocale | null {
...
export function resolveRequestedLocale(request: Request): ArticleLocale {
...
export async function listArticleItems(
  bucket: R2Bucket,
  locale: ArticleLocale
): Promise<ArticleListItem[]> {
  const sources = await listArticleSources(bucket, locale);

  const summaries = await Promise.all(
    sources.map(async (source) => {
      try {
        const loaded = await loadArticleBySource(bucket, source);
        return toArticleListItem(loaded.item);
      } catch {
        return null;
      }
    })
...
export async function loadArticleById(
  ...
  articleId: string,
  locale: ArticleLocale
): Promise<LoadedArticleDetail | null> {
  const source = await findArticleSourceById(bucket, articleId, locale);
  if (!source) {
    return null;
  }
  return loadArticleBySource(bucket, source);
}

export function articleDetailToMarkdown(item: ArticleDetailItem): string {
  const lines: string[] = [];
  lines.push("---");
  lines.push(`title: "${escapeYaml(item.title)}"`);
  lines.push(`summary: "${escapeYaml(item.summary ?? "")}"`);
  lines.push(`publishedAt: "${escapeYaml(item.publishedAt)}"`);
  if (item.updatedAt) {
    lines.push(`updatedAt: "${escapeYaml(item.updatedAt)}"`);
  }
  lines.push(`category: "${escapeYaml(item.category)}"`);
  lines.push(
    `tags: [${item.tags.map((tag) => `"${escapeYaml(tag)}"`).join(", ")}]`
  );
  lines.push("author:");
  lines.push(`  - "${escapeYaml(item.author.name)}"`);
  lines.push(`authorRole: "${escapeYaml(item.author.role)}"`);
  if (item.author.profileUrl) {
    lines.push(`authorUrl: "${escapeYaml(item.author.profileUrl)}"`);
  }

  for (const link of item.links) {
    const key = inferFrontMatterLinkKey(link.label);
    if (!key) {
      continue;
    }
    lines.push(`${key}: "${escapeYaml(link.href)}"`);
  }
  ...
  if (item.links.length > 0) {
    ...
    lines.push("");
    for (const link of item.links) {
      const description = link.description?.trim() ? ` - ${link.description}` : "";
      lines.push(`- [${link.label}](${link.href})${description}`);
    }
    lines.push("");
  }
...
async function listArticleSources(
  bucket: R2Bucket,
  locale: ArticleLocale
): Promise<ArticleSourceDescriptor[]> {
...
async function findArticleSourceById(
  bucket: R2Bucket,
  articleId: string,
  locale: ArticleLocale
): Promise<ArticleSourceDescriptor | null> {
  ...
  for (const extension of ARTICLE_EXTENSIONS) {
    ...
    if (object) {
      return { key, id: normalizedId, extension, locale };
    }
  }
...
async function loadArticleBySource(
  bucket: R2Bucket,
  source: ArticleSourceDescriptor
): Promise<LoadedArticleDetail> {
  ...
  const sourceEtag = object.etag ?? object.httpEtag ?? "unknown";
  let item: ArticleDetailItem;
  try {
    item =
      source.extension === ".md"
        ? parseMarkdownArticle(source.id, rawText)
        : parseJsonArticle(source.id, rawText);
  } catch (error) {
    console.error("Failed to parse article source", {
      articleId: source.id,
  ...
  const sourceMarkdown =
    source.extension === ".md" ? normalizeMarkdownSource(rawText) : articleDetailToMarkdown(item);
...
function inferFrontMatterLinkKey(label: string): string | null {
