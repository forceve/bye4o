...
type UnburntDetailRoutePath = `/unburnt/${string}`;
type RoutePath = StaticRoutePath | ArticleDetailRoutePath | UnburntDetailRoutePath;
...
function buildUnburntDetailRoute(unburntId: string): UnburntDetailRoutePath {
...
function extractUnburntId(path: string): string | null {
...
function isUnburntDetailRoute(route: RoutePath): route is UnburntDetailRoutePath {
...
function getGlobalBackFallbackRoute(route: RoutePath): RoutePath {
  ...
  if (isUnburntDetailRoute(route) || route === ROUTES.unburntMine || route === ROUTES.unburntNew) {
...
function getNavActivePath(route: RoutePath): AppRoute {
  ...
  if (route === ROUTES.unburntMine || route === ROUTES.unburntNew || isUnburntDetailRoute(route)) {
    return ROUTES.unburnt;
  }
...
function renderUnburntMinePage(): string {
  return `
    <section class="page-intro page-intro--split">
      <div class="page-intro__main">
        <p class="page-kicker">${copy.unburnt.kicker}</p>
...
function renderUnburntComposerPage(): string {
...
function renderUnburntDetailPage(route: UnburntDetailRoutePath): string {
  const unburntId = extractUnburntId(route) ?? "";
...
async function setupUnburntMinePage() {
  ...
  const handleMineListClick = (event: Event) => {
    ...
    if (editId) {
      navigate(buildUnburntDetailRoute(editId));
      return;
    }
...
async function setupUnburntDetailPage(route: UnburntDetailRoutePath) {
  ...
  const renderReadOnly = (item: UnburntEntryDetail) => {
    const visibilityLabel =
      item.visibility === "public"
        ? copy.unburnt.listVisibilityPublic
        : copy.unburnt.listVisibilityPrivate;
  ...
  const mountOwnerEditor = (item: UnburntEntryDetail) => {
    const visibilityLabel =
      item.visibility === "public"
        ? copy.unburnt.listVisibilityPublic
        : copy.unburnt.listVisibilityPrivate;
    ...
    body.innerHTML = `
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="${copy.unburnt.detailSaveMetaButton}"
        aria-hidden="true"
      >
        <div class="unburnt-preview-modal__backdrop" data-unburnt-preview-close="true"></div>
        <section class="unburnt-preview-modal__panel">
          ...
          <div class="unburnt-preview-modal__actions">
            <button id="${UNBURNT_PREVIEW_MODAL_CONFIRM_ID}" type="button" class="action-button">
              ${copy.unburnt.detailSaveMetaButton}
            </button>
          </div>
        </section>
    ...
    const closePreviewModal = (restoreFocus = false) => {
      ...
      previewModal.setAttribute("aria-hidden", "true");
      document.body.classList.remove("unburnt-preview-modal-open");
      if (restoreFocus && saveButton.isConnected) {
        saveButton.focus();
      }
    };
    ...
    const syncPreview = () => {
      ...
      const tagsMarkup = tags.length
        ? tags.map((tag) => `<span class="unburnt-tag">${escapeHtml(tag)}</span>`).join("")
        : `<span class="unburnt-tag unburnt-tag--muted">-</span>`;
      ...
      previewCard.innerHTML = `
        <article class="fragment-card fragment-card--unburnt">
          ...
          <div class="fragment-card__meta">
            <span>${escapeHtml(visibilityLabel)}</span>
            <span>${escapeHtml(copy.unburnt.listMessageCount(item.messages.length))}</span>
          </div>
          <div class="unburnt-tags">${tagsMarkup}</div>
        </article>
      `;
      previewDetail.innerHTML = renderMessages(item.messages);
    };
    ...
    previewModalConfirmButton.addEventListener("click", onPreviewConfirm);
    cleanupTasks.push(() => {
      previewModalConfirmButton.removeEventListener("click", onPreviewConfirm);
    });
